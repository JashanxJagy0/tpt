...
# basket_games[user_id] = {
#    bet, mode, to_win, stage,
#    round_no, initial_first,
#    score: {user,bot},
#    bot_total (list), user_shots,
#    chat_id, msg_id
# }
basket_games = {}
...
def build_mode_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Normal Mode",   callback_data="bask_mode:normal")],
        [InlineKeyboardButton("Double Shot",   callback_data="bask_mode:double")],
        [InlineKeyboardButton("Crazy Mode",    callback_data="bask_mode:crazy")],
        [
            InlineKeyboardButton("ℹ Mode Guide", callback_data="bask_mode:guide"),
            InlineKeyboardButton("❌ Cancel",     callback_data="bask_cancel")
        ]
    ])

def build_points_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("First to 3 points", callback_data="bask_points:3")],
        [InlineKeyboardButton("First to 2 points", callback_data="bask_points:2")],
        [InlineKeyboardButton("First to 1 point",  callback_data="bask_points:1")],
        [InlineKeyboardButton("❌ Cancel",         callback_data="bask_cancel")]
    ])

def build_confirm_keyboard():
    return InlineKeyboardMarkup([[ 
        InlineKeyboardButton("✅ Confirm", callback_data="bask_confirm:yes"),
        InlineKeyboardButton("❌ Cancel",  callback_data="bask_cancel")
    ]])
...
def build_accept_keyboard():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Accept Match",     callback_data="bask_accept:yes"),
            InlineKeyboardButton("✅ Play against bot", callback_data="bask_accept:bot")
        ],
        [InlineKeyboardButton("❌ Cancel", callback_data="bask_cancel")]
    ])
...
def build_end_keyboard(bet: float, payout: float):
    return InlineKeyboardMarkup([[ 
        InlineKeyboardButton("🔄 Play Again", callback_data=f"bask_replay:{bet}"),
        InlineKeyboardButton("✖ Double",      callback_data=f"bask_double:{payout}")
    ]])
...
async def bask_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        return await update.message.reply_text(
            "🏀 *Play Basketball*\n\n"
            "To play, type the command /bask with the desired bet.\n\n"
            "Examples:\n"
            "/bask 5.50 - to play for $5.50\n"
            "/bask half - to play for half your balance\n"
...
async def bask_confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["mode"] == "normal":
        desc = (
            "*Normal Mode*\n"
            "Basic game mode. You take turns shooting and higher total wins the round.\n\n"
        )
    elif state["mode"] == "double":
        desc = (
            "*Double Shot*\n"
            "Two shots in a row. The higher sum wins the round.\n\n"
        )
    else:
        desc = (
            "*Crazy Mode*\n"
    ...
    await q.edit_message_text(
        "🏀 {name} wants to play Basketball!\n\n"
        "Bet: <b>${bet:,.2f}</b>\n"
        "Win multiplier: <b>1.92×</b>\n"
        "Mode: First to <b>{tw}</b> point{s}\n\n"
        "<b>{mode} Mode</b>\n"
        "Basic game mode. You take turns shooting, and whoever has the higher total wins the round.\n\n"
...
# ─────────────────────────────────────────────────────────────────────────────
async def bask_start_round(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    state   = basket_games[user_id]
    chat_id = state["chat_id"]
    rn      = state["round_no"]
    init    = state["initial_first"]

    # Who shoots first this round?
    shooter = init if (rn % 2) == 1 else ("bot" if init == "user" else "user")

    if shooter == "bot":
        # Bot shoots first
        shot_ct = 2 if state["mode"] == "double" else 1
        shots = []
        for _ in range(shot_ct):
            dmsg = await helper_bot.send_dice(chat_id=chat_id, emoji="🏀")
            await asyncio.sleep(1)
            shots.append(dmsg.dice.value)
        state["bot_total"] = shots if shot_ct > 1 else shots[0]
        state["bot_has_shot"] = True

        # Now ping the user to shoot — reply to their last 🏀, use full name
...
# ─────────────────────────────────────────────────────────────────────────────
async def handle_user_shots(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handles user's 🏀 basketball messages during an active match.
    - Records user shot(s)
    - If user goes first -> replies "Bot, your turn!" to the user's 🏀 and makes the bot shoot
    - If bot goes first -> after bot shot it replies to the user's last message: "<you>, your turn!"
    ...
    state["last_user_bask_msg_id"] = msg.message_id

    # Record user's shot(s)
    state.setdefault("user_shots", [])
    state["user_shots"].append(msg.dice.value)
    needed = 2 if state["mode"] == "double" else 1

    # If double-shot and only 1 shot received so far, ask for the second shot
    if len(state["user_shots"]) < needed:
        return await msg.reply_text(
            f"<b>{msg.from_user.full_name}</b>, one more shot!",
            parse_mode="HTML"
        )

    # Determine who should have shot first this round
    rn = state["round_no"]
    first_shooter = state["initial_first"] if (rn % 2) == 1 else ("bot" if state["initial_first"] == "user" else "user")

    # ========== When user goes first: make bot shoot now and thread the prompt ==========
    if first_shooter == "user" and not state.get("bot_total"):
        await context.bot.send_message(
            chat_id=state["chat_id"],
            text="Bot, your turn!",
            parse_mode="HTML",
            reply_to_message_id=state["last_user_bask_msg_id"]
        )
        bot_shots = []
        for _ in range(needed):
            dmsg = await helper_bot.send_dice(chat_id=state["chat_id"], emoji="🏀")
            await asyncio.sleep(1)
            bot_shots.append(dmsg.dice.value)
        state["bot_total"] = bot_shots if needed > 1 else bot_shots[0]
        state["bot_has_shot"] = True

    # ========== When bot goes first but hasn't shot yet (user shot prematurely) ==========
    if first_shooter == "bot" and not state.get("bot_total"):
        # Ask the bot to shoot first, then tell the user "Your turn!" replying to user's 🏀
        await context.bot.send_message(
            chat_id=state["chat_id"],
            text="Bot, your turn!",
            parse_mode="HTML",
    ...
    # ========== Both have shot -> decide the round ==========
    ut = sum(state["user_shots"])
    bt = state["bot_total"]
    ...
    if state["mode"] == "crazy":
        ut = sum(7 - v for v in state["user_shots"])
    ...
    if ut == bt:
        round_text = (
            ...
            f"{msg.from_user.full_name} shot <b>{ut}</b>\n"
            f"🤖 Bot shot <b>{bt}</b>\n\n"
            "🤝 <b>It’s a draw!</b>\n"
        )
    else:
    ...
    state["round_no"] += 1
    state["user_shots"].clear()
    state["bot_total"] = []
    state["bot_has_shot"] = False
    ...
    if next_first == "user":
        ...
        sent = await context.bot.send_message(
            ...
            parse_mode="HTML",
            reply_to_message_id=state.get("last_user_bask_msg_id"),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(cash_label, callback_data="bask_action:cashout")]])
        )
        state["last_score_msg_id"] = sent.message_id
    else:
        # Tell bot to shoot immediately for next round (and keep it threaded)
        await context.bot.send_message(
            chat_id=state["chat_id"],
            text=score_text,
            parse_mode="HTML",
        ...
        # Bot actually shoots now for the next round
        bot_shots = []
        for _ in range(needed):
            dmsg = await helper_bot.send_dice(chat_id=state["chat_id"], emoji="🏀")
            await asyncio.sleep(1)
        ...
        # After bot finishes shooting first, ping the user under their last 🏀
        await context.bot.send_message(
            chat_id=state["chat_id"],
            text=f"{member.user.full_name}, your turn!",
            reply_to_message_id=state.get("last_user_bask_msg_id"),
...
async def bask_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "next":
        ...
        for _ in range(shot_ct):
            ...
            shots.append(dmsg.dice.value)
        # store raw shots list
        state["bot_total"] = shots
